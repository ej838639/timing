...
class EventEngine:
    ...
    def on_servo(self, s: ServoSample) -> list[Event]:
        ev: list[Event] = []
        self._last_servo_t = s.t_monotonic
        self._lost_sync_active = False

        off_ms = abs(ns_to_ms(s.offset_ns))
        if off_ms >= self.th.alarm_ms:
            ev.append(Event(s.t_monotonic, Severity.ALARM, "offset_alarm", f"Offset {off_ms:.3f} ms >= {self.th.alarm_ms} ms"))
        elif off_ms >= self.th.warn_ms:
            ev.append(Event(s.t_monotonic, Severity.WARN, "offset_warn", f"Offset {off_ms:.3f} ms >= {self.th.warn_ms} ms"))

        # Simple heuristic: if servo state not in tighter states, note it
        if s.state not in ("s0", "s1"):
            ev.append(Event(s.t_monotonic, Severity.INFO, "servo_state", f"Servo state {s.state}"))
        return ev
    ...
    def check_lost_sync(self, now_t: float) -> Optional[Event]:
        ...
        if dt >= self.th.lost_sync_seconds and not self._lost_sync_active:
            ...
            return Event(now_t, Severity.ALARM, "lost_sync", f"No servo updates for {dt:.1f}s (>= {self.th.lost_sync_seconds}s)")
        return None
